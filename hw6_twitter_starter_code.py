...
def make_request(baseurl, params):
    ...
    '''
    #TODO Implement function
    response_dict = requests.get(baseurl,params, auth=oauth).json()
...
def find_most_common_cooccurring_hashtag(tweet_data, hashtag_to_ignore):
    ...
    for tweet in tweet_data["statuses"]:
        tweet_entity = tweet["entities"]
        tweet_hashtags = tweet_entity["hashtags"]

        for each_hashtag in tweet_hashtags:
            hashtag_text = each_hashtag["text"]
            hashtag_text = hashtag_text.lower()
            if hashtag_text != hashtag_to_ignore.lower():
                if hashtag_text in hashtag_dict.keys():
                    hashtag_dict[hashtag_text] += 1
                else:
                     hashtag_dict[hashtag_text] = 1

    hashtag_max = 0
    for hg, num in hashtag_dict.items():
        if num >= hashtag_max:
            hashtag_cooccur = hg
            hashtag_max = num

    final_hashtag = f"{hashtag_cooccur}"
